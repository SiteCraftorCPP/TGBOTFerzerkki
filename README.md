...
Если **сменил чат модерации**, новый `MODERATION_CHAT_ID` читается из `.env` при следующем обращении (кэш настроек убран); сохранённая подтема пользователя сбросится и бот создаст тему в **новой** группе (нужны права «Управление темами»). Смена **токена** бота по-прежнему требует `systemctl restart …`.
