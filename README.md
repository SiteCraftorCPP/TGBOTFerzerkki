...
Если **сменил чат модерации**, новый `MODERATION_CHAT_ID` подхватится из `.env` без перезапуска; сохранённая подтема пользователя сбросится и бот создаст тему в **новой** группе (нужны права «Управление темами»).
